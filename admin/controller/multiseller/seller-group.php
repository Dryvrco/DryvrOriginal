...
class ControllerMultisellerSellerGroup extends ControllerMultisellerBase {
    ...
    public function getTableData() {
        ...
        foreach ($results as $result) {
            ...
            $actions .= "&nbsp;<a class='btn btn-danger' href='" . $this->url->link('multiseller/seller-group/delete', 'token=' . $this->session->data['token'] . '&seller_group_id=' . $result['seller_group_id'], 'SSL') . "' title='" . $this->language->get('button_delete') . "' OnClick=\"return confirm('Are you sure?');\"><i class='fa fa-trash-o'></i></a>";
    ...
    public function jxSave() {
        ...
        if (!isset($data['product_period']) || empty($data['product_period']))
            $data['product_period'] = 0;
        ...
        if (!empty($data['seller_group_id']) && $this->config->get('msconf_default_seller_group_id') == $data['seller_group_id']) {
            foreach ($data['commission_rates'] as &$rate) {
                ...
                if (empty($rate['percent']))
                    $rate['percent'] = 0;
                if (!isset($rate['payment_method']) || (int) $rate['payment_method'] == 0)
                    $rate['payment_method'] = 1;
            }
            unset($rate);
        }
