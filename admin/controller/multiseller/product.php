...
class ControllerMultisellerProduct extends ControllerMultisellerBase {
    ...
    public function getTableData() {
        ...
        $sorts = array('seller', 'date_created', 'date_modified', 'status');
        ...
        foreach ($results as $result) {
            ...
            if ($result['p.make_id']) {
                $make = $this->model_catalog_category->getCategory($result['p.make_id']);
                $detail = $make['name'];
            }

            if ($result['p.model_id']) {
                $model = $this->model_catalog_category->getCategory($result['p.model_id']);
                $detail .= ' ' . $model['name'];
            }
            ...
            $actions .= "&nbsp;<a class='btn btn-danger' href='" . $this->url->link('multiseller/product/delete', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'], 'SSL') . "' title='" . $this->language->get('ms_delete') . "' OnClick=\"return confirm('Are you sure?');\"><i class='fa fa-trash-o'></i></a>";
            ...
            $columns[] = array_merge(
                ...
                'name' => $detail . $year,
                'seller' => $sellerselect,
                'status' => $result['mp.product_status'] ? $this->language->get('ms_product_status_' . $result['mp.product_status']) : 'Disabled',
    ...
    public function index() {
        ...
        $this->data['add'] = $this->url->link('catalog/product/preadd', 'token=' . $this->session->data['token'] . $url, 'SSL');
