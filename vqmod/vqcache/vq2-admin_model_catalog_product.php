...
class ModelCatalogProduct extends Model {
    ...
    public function addProduct($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "ms_product SET product_id = '" . (int) $product_id . "', seller_id = '" . $this->db->escape($data['agency_id']) . "', product_status = '" . (int) $data['status'] . "', product_approved = '1'");
    ...
    public function getCityName($city_id) {
        $sql = "SELECT name FROM " . DB_PREFIX . "city WHERE city_id = '" . $city_id . "'";
        $query = $this->db->query($sql);
    ...
    public function editProduct($product_id, $data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', model_id = '" . $this->db->escape($data['model_id']) . "', make_id = '" . $this->db->escape($data['make_id']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '999', minimum = '" . (int) $data['minimum'] . "', subtract = '" . (int) $data['subtract'] . "', stock_status_id = '" . (int) $data['stock_status_id'] . "', manufacturer_id = '" . (int) $data['manufacturer_id'] . "', shipping = '0', price = '" . (float) $data['price'] . "', points = '" . (int) $data['points'] . "', weight = '" . (float) $data['weight'] . "', weight_class_id = '" . (int) $data['weight_class_id'] . "', length = '" . (float) $data['length'] . "', width = '" . (float) $data['width'] . "', height = '" . (float) $data['height'] . "', length_class_id = '" . (int) $data['length_class_id'] . "', status = '" . (int) $data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int) $data['sort_order'] . "', date_modified = NOW(), cat_id = '" . (int) $data['cat_id'] . "', subcat_id = '" . $data['subcat_id'] . "', daily = '" . $data['daily'] . "', weekly = '" . $data['weekly'] . "', weekend = '" . $data['weekend'] . "', monthly = '" . $data['monthly'] . "', insurance = '" . $data['insurance'] . "' , mileage = '" . $data['mileage'] . "', over_miles = '" . $data['over_miles'] . "', delivery = '" . $data['delivery'] . "', airport = '" . $data['airport'] . "', after_hours = '" . $data['after_hours'] . "', security = '" . $data['security'] . "', tags = '" . $data['tags'] . "', city_id = '" . $data['city_id'] . "', country_id = '" . $data['country_id'] . "', min_age = '" . $data['min_age'] . "' WHERE product_id = '" . (int) $product_id . "'");

        $this->db->query("UPDATE " . DB_PREFIX . "ms_product SET seller_id = '" . $this->db->escape($data['agency_id']) . "', product_status = '" . (int) $data['status'] . "' WHERE product_id = '" . (int) $product_id . "'");
    ...
    public function deleteProduct($product_id) {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_recurring WHERE product_id = " . (int) $product_id);
        $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int) $product_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_available WHERE product_id = '" . (int) $product_id . "'");
        $this->cache->delete('product');
    ...
    public function getAgencies() {
        $sql = "SELECT mss.seller_id,mss.nickname FROM " . DB_PREFIX . "ms_seller mss LEFT JOIN " . DB_PREFIX . "customer c on (c.customer_id = mss.seller_id) WHERE c.customer_id=c.agency_id AND mss.seller_status = '1'";
        $query = $this->db->query($sql);
        foreach ($query->rows as $result) {
    ...
    public function getTotalAgencies() {
        $sql = "SELECT count(mss.seller_id) as total FROM " . DB_PREFIX . "ms_seller mss LEFT JOIN " . DB_PREFIX . "customer c on (c.customer_id = mss.seller_id) WHERE c.customer_id=c.agency_id";
        $query = $this->db->query($sql);
        return $query->row['total'];
    }
    ...
    public function getAgencyName($product_id) {
        $sql = "SELECT mss.nickname FROM " . DB_PREFIX . "ms_seller mss LEFT JOIN " . DB_PREFIX . "ms_product msp ON (mss.seller_id=msp.seller_id) WHERE msp.product_id = '" . $product_id . "'";
        $query = $this->db->query($sql);
        return $query->row['nickname'];
    }
    
    public function getMakeId($model_id) {
        $sql = "SELECT parent_id FROM " . DB_PREFIX . "category WHERE category_id = '" . $model_id . "'";
        $query = $this->db->query($sql);
