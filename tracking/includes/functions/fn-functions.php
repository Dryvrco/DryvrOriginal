...
function check_package( $package ) {
	...
	switch ( $package ) {
		case "agency":
			return "Heat Map Tracker Agency";
			break;
		case "personal":
			return "Heat Map Tracker Personal";
			break;
	}
}
...
function add_option( $name, $data ) {
	global $wpdb;
	$table_options = T_PREFIX . "options";
	$line          = $wpdb->query( "INSERT INTO $table_options (`data`,`name`) VALUES ('" . $wpdb->escape_String( serialize( $data ) ) . "', '" . $name . "')" );
...
function update_option( $name, $data ) {
	global $wpdb;
	$table_options = T_PREFIX . "options";
	$query         = "UPDATE $table_options SET`data`='" . $wpdb->escape_String( serialize( $data ) ) . "' WHERE `name` = '" . $name . "'";
	$line          = $wpdb->query( $query );
...
function requestUri( $base_dir ) {
	$request_uri = preg_replace( '%' . basename( $_SERVER['PHP_SELF'] ) . '.*%', '', $_SERVER['REQUEST_URI'] );
	if ( $pos = strpos( $request_uri, "?" ) ) {
		$request_uri = substr( $request_uri, 0, $pos );
	}
	if ( substr( $request_uri, 0, strlen( $base_dir ) ) == $base_dir ) {
		return rtrim(substr( $request_uri, strlen( $base_dir ) ), "/");
	}

	return '';

}
function siteURL( $base_url = true, $trailing_slash = true, $makessl = false, $pathonly = false, $request_uri = false ) {
	$pageURL = 'http';
...
//secure post variables
	public static function hmtracker_secure( $string ) {
		...
		$string = $wpdb->escape_String( $string );
...
function is_agency() {
	global $HMTrackerPro_PACKAGE;

	return $HMTrackerPro_PACKAGE == "agency";
}

function is_personal() {
	global $HMTrackerPro_PACKAGE;

	return $HMTrackerPro_PACKAGE == "personal";
}