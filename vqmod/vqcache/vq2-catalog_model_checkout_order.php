		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
					...
					foreach ($order_option_query->rows as $option) {
						...
						$option_data[] = array(
							...
							'value' => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value)
                                        ...
                                        $agency_nick = $this->model_catalog_product->getSellerNickname($product['product_id']);
				...
				'seller_text' => $seller ? "<br/ > " . $this->language->get('ms_by') . " {$seller['ms.nickname']} <br />" : '',
						...
						'name'     => $this->model_catalog_product->getMakeModelCategories($product['product_id']),
						'model'    => $product['model'],
						'option'   => $option_data,
						...
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
                                        ...
                                        $agency_admin = $this->model_catalog_product->getSellerEmail($product['product_id']);
                                        $seller = $this->model_catalog_product->getSellerId($product['product_id']);
                                        
                                        $this->db->query("INSERT INTO ".DB_PREFIX."ms_order_product_data SET order_id = '".$order_id."', product_id = '".$product['product_id']."', seller_id = '".$seller."', seller_net_amt = '".$product['total']."'");
                                        $this->db->query("INSERT INTO ".DB_PREFIX."ms_suborder SET order_id = '".$order_id."', seller_id = '".$seller."', order_status_id = '0'");
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/order.tpl')) {
					...
					$html = $this->load->view('default/template/mail/order.tpl', $data);
                                ...
                                // HTML1 Mail
                                $subject1 = sprintf($language->get('text_new_subject'), $agency_nick, $order_id);
                                
				$data = array();
                                
                                $data['res_start'] = $this->session->data['dsd'].' '.$this->session->data['dst'];
                                $data['res_end'] = $this->session->data['ded'].' '.$this->session->data['det'];
                                
                                $insurance_info = $this->model_account_customer->getCustomerInsurance($order_info['customer_id']);
                                $data['driver_license'] = $insurance_info['driver_license'];
                                $data['dl_expiry_mm'] = $insurance_info['dl_expiry_mm'];
                                $data['dl_expiry_yy'] = $insurance_info['dl_expiry_yy'];
                                $data['ins_company_name'] = $insurance_info['ins_company_name'];
                                $data['insurance_info'] = $insurance_info['insurance_info'];
                                $data['ins_agent_name'] = $insurance_info['ins_agent_name'];
                                $data['ins_policy_number'] = $insurance_info['ins_policy_number'];
                                $data['ins_expiry_mm'] = $insurance_info['ins_expiry_mm'];
                                $data['ins_expiry_yy'] = $insurance_info['ins_expiry_yy'];

				$data['title'] = sprintf($language->get('text_new_received'), html_entity_decode($agency_nick, ENT_QUOTES, 'UTF-8'), $order_id);

				$data['text_greeting'] = sprintf($language->get('text_new_received'), html_entity_decode($agency_nick, ENT_QUOTES, 'UTF-8'));
				$data['text_link'] = $language->get('text_new_link');
				$data['text_download'] = $language->get('text_new_download');
				$data['text_order_detail'] = $language->get('text_new_order_detail');
				$data['text_instruction'] = $language->get('text_new_instruction');
				...
				$data['store_url'] = $order_info['store_url'];
				$data['customer_id'] = $order_info['customer_id'];
				$data['link'] = $order_info['store_url'] . 'index.php?route=seller/account-order/viewOrder&order_id=' . $order_id;
				...
				foreach ($order_total_query->rows as $total) {
					$data['totals'][] = array(
						...
						'text'  => $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']),
                                ...
                                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/agency_order.tpl')) {
					$html1 = $this->load->view($this->config->get('config_template') . '/template/mail/agency_order.tpl', $data);
				} else {
					$html1 = $this->load->view('default/template/mail/agency_order.tpl', $data);
				}
				...
				$data = array();
                                ...
                                $data['dl_expiry_yy'] = $insurance_info['dl_expiry_yy'];
                                $data['ins_company_name'] = $insurance_info['ins_company_name'];
                                $data['ins_info'] = $insurance_info['insurance_info'];
                                $data['ins_agent_name'] = $insurance_info['ins_agent_name'];
                                $data['ins_policy_number'] = $insurance_info['ins_policy_number'];
                                $data['ins_expiry_mm'] = $insurance_info['ins_expiry_mm'];
                                $data['ins_expiry_yy'] = $insurance_info['ins_expiry_yy'];
				...
				foreach ($order_total_query->rows as $total) {
					$data['totals'][] = array(
						...
						'text'  => $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']),
                                ...
                                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/agency_order.tpl')) {
					$html2 = $this->load->view($this->config->get('config_template') . '/template/mail/agency_order.tpl', $data);
				} else {
					$html2 = $this->load->view('default/template/mail/agency_order.tpl', $data);
				}
				...
				if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
					...
					$mail->send();
                                        ...
                                        $sql = "SELECT email FROM ".DB_PREFIX."user WHERE status = '1'";
