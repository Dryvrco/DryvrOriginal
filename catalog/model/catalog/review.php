...
class ModelCatalogReview extends Model {
    ...
    public function addReview($product_id, $data) {
        ...
        $seller_id_query = $this->db->query("SELECT seller_id FROM " . DB_PREFIX . "ms_product WHERE product_id = '" . (int) $product_id . "'");
        $seller_id = $seller_id_query->row['seller_id'];

        $this->db->query("UPDATE " . DB_PREFIX . "review SET agency_id = '" . $seller_id . "'");
    ...
    public function getReviews($seller_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "ms_product msp ON (r.product_id = msp.product_id) WHERE msp.seller_id = '" . (int) $seller_id . "' AND r.status = '1'");
        return $query->rows;
    }
    ...
    public function getTotalReviews($seller_id) {
        $query = $this->db->query("SELECT count(r.review_id) as total FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "ms_product msp ON (r.product_id = msp.product_id) WHERE msp.seller_id = '" . (int) $seller_id . "' AND r.status = '1'");
        return $query->row['total'];
    }
