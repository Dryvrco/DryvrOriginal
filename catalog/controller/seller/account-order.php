...
class ControllerSellerAccountOrder extends ControllerSellerAccount {
    ...
    public function getTableData() {
        ...
        foreach ($orders as $order) {
            ...
            $columns[] = array_merge(
                ...
                'products' => "{$this->model_localisation_order_status->getOrderProducts($order['order_id'])}",
                'suborder_status' => $order_status_name,
                'date_created' => date($this->language->get('date_format_short'), strtotime($order['date_added'])),
                'booking_time' => 'From: ' . date("m-d-Y", strtotime($order['start_date'])) . ' ' . date("h:i A", strtotime($order['start_time'])) . '<br>' . 'To: ' . date("m-d-Y", strtotime($order['end_date'])) . ' ' . date("h:i A", strtotime($order['end_time'])),
    ...
    public function viewOrder() {
        ...
        if (!$this->agency->isLogged()) {
            $this->response->redirect($this->url->link('account/agencylogin', '', 'SSL'));
        }
        ...
        $order_info = $this->model_account_vieworder->getOrder($order_id, 'seller');

        $this->data['products'] = array();
        $products = $this->MsLoader->MsOrderData->getOrderProducts(array('order_id' => $order_id, 'seller_id' => $this->agency->getSellerId()));
        ...
        if ($order_info && !empty($products)) {
            ...
            foreach ($products as $product) {
				...
				$options = $this->model_account_vieworder->getOrderOptions($order_id);
