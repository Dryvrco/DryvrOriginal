		...
		<div class="table-holder">
			...
			<form action="#" method="POST" class="form-horizontal" id="">
				...
				<?php if ( $updackage ) { ?>
					...
					if ( $user_status != USER_STATUS_FREE_ACTIVE ) {
						?>
						<div class="control-group">
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<?php
						if ( ! ( isset( $this->PACKAGES[ $updackage['pack_id'] ] ) && count( $this->PACKAGES ) == 1 ) && has_available_packages( $this->PACKAGES, $this->PROJECTS ) ) { ?>
							<a href="<?php echo $this->PLUGIN_URL ?>?changepackage" class="btn btn-primary btn-mini">Change Package</a>
						<?php } ?>
						<?php if ( $updackage && $user_status != USER_STATUS_FREE_ACTIVE ) { ?>
			...
			<form id="to_del_form"
				...
				<table border="0" width="100%" cellpadding="0" cellspacing="0"
					...
					<?php
					global $wpdb;
					$search = " WHERE `user_id` = '" . $user->id . "' AND `txn_type`  IN('payment','refund', 'manual', 'cancel')";
					$q      = "SELECT * FROM `" . T_PREFIX . $this->OPTIONS['dbtable_name_payments'] . "` " . $search;

					//pagination
					$perpage = ( isset( $_GET['perpage'] ) ) ? $wpdb->escape_String( $_GET['perpage'] ) : 30;
