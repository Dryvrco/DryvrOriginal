...
class ControllerAccountRegisterSeller extends Controller {
    ...
    public function jxsavesellerinfo() {
        ...
        if ((utf8_strlen($data['seller']['password']) < 3)) {
            $json['errors']['seller[password]'] = "Please type a valid password.";
        }

        /* if (!preg_match('^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&amp;+=])(?=\S+$).{8,}$', $data['seller']['password'])) {
          $json['errors']['seller[password]'] = "Please ensure that your password contains a number, a lowercase letter, and upper case letter, a symbol and is longer than eight characters";
          } */
        ...
        if (empty($json['errors'])) {
            ...
            $seller_id = $this->MsLoader->MsSeller->createSeller($data['seller']);
            ...
            $this->agency->login($data['seller']['email'], $data['seller']['password']);
        }
