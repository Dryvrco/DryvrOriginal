...
class ModelCatalogFilter extends Model {
    ...
    public function getAllFilterGroups() {
        $sql = "SELECT fg.filter_group_id,fgd.name FROM `" . DB_PREFIX . "filter_group` fg LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON (fg.filter_group_id = fgd.filter_group_id) WHERE fgd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND fg.filter_group_id != '5'";
        $query = $this->db->query($sql);
        return $query->rows;
    }

    public function getFilters($filter_group_id) {
        $sql = "SELECT f.filter_id,fd.name FROM " . DB_PREFIX . "filter f LEFT JOIN " . DB_PREFIX . "filter_description fd ON (f.filter_id = fd.filter_id) WHERE fd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND f.filter_group_id = '" . (int) $filter_group_id . "'";
    ...
    public function getAgencyFiltersList($agency_id) {
        $product_filter_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "agency_filter WHERE agency_id = '" . (int) $agency_id . "'");
