...
class ControllerProductProduct extends Controller {
    ...
    public function index() {
        ...
        $product_info = $this->model_catalog_product->getProductDetail($product_id);

        if ($this->request->get['start_date'] != '' && $this->request->get['start_time'] != '' && $this->request->get['end_date'] != '' && $this->request->get['end_time'] != '') {
            $data['available'] = $this->model_catalog_product->showVehlicle($product_id, $unix_start, $unix_end);
        } else {
            $data['available'] = 2;
        }
        ...
        $data['agencyurl'] = $this->url->link('agency/agency_detail&agency_id=' . $product_info['seller_id']);
        ...
        if ($product_info) {
            ...
            if ($product_info['image']) {
                $data['photo_thumb'] = $this->model_tool_image->resize($product_info['image'], 800, 364);
            } else {
                $data['photo_thumb'] = $this->model_tool_image->resize('no_image.jpg', 800, 364);
            }

            $data['images'] = array();

            $data['breadcrumbs'][] = array(
                'text' => $product_info['nickname'],
                'href' => $this->url->link('agency/agency_detail', '&agency_id=' . $product_info['seller_id'])
            );

            $data['breadcrumbs'][] = array(
                'text' => $this->model_catalog_product->getMakeModelYear($product_info['product_id']),
            ...
            $data['rate'] = $this->currency->format(calculaterate($this->session->data['booking_hours'], $product_info['daily'], $product_info['weekly'], $product_info['monthly']));

            $data['product_id'] = $product_info['product_id'];
            $data['description'] = $product_info['description'];
            $data['daily'] = $this->currency->format($product_info['daily']);
            $data['weekly'] = $this->currency->format($product_info['weekly']);
            $data['monthly'] = $this->currency->format($product_info['monthly']);
            $data['name'] = $this->model_catalog_product->getMakeModelYear($product_info['product_id']);
            $data['options'] = $this->model_catalog_product->getProductOptions($product_info['product_id']);
            $data['filters'] = $this->model_catalog_product->getProductFiltersNames($product_info['product_id']);
            $data['nickname'] = $product_info['nickname'];
            $data['insurance'] = $product_info['insurance'];
            $data['security'] = $this->currency->format($product_info['security']);
            $data['min_age'] = $product_info['min_age'];
            $data['agencyrating'] = (int) $this->model_agency_agency->getAgencyReview($product_info['seller_id']);
            $data['agency_country'] = $product_info['country_name'];
            $data['agency_zone'] = $product_info['zone_name'];
            $data['airport'] = $this->currency->format($product_info['airport']);
            $data['delivery'] = $this->currency->format($product_info['delivery']);
            ...
            if ($product_info['avatar']) {
                $data['avatar'] = $this->model_tool_image->resize($product_info['avatar'], 200, 200);
            } else {
                $data['avatar'] = $this->model_tool_image->resize('no_image.jpg', 200, 200);
            }
