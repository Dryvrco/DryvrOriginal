...
class ModelLocalisationOrderStatus extends Model {
    ...
    public function getOrderProducts($order_id) {
        $sql = "SELECT cd.name,p.model FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "product p ON (op.product_id=p.product_id) LEFT JOIN " . DB_PREFIX . "category_description cd on (p.make_id=cd.category_id) AND op.order_id = '" . $order_id . "'";
        $query = $this->db->query($sql);
        $gap = '';
        foreach ($query->rows as $model) {
            if ($model['name'] != '' && $model['model'] != '') {
                $res .= $gap . $model['name'] . ' ' . $model['model'];
                $gap = '<br />';
            }
        }
        return $res;
