...
class hmtracker_GeoIP {
	...
	private function search( $beg, $end ) {
		...
		if ( ( $this->_longip >= $arr_beg[2] ) && ( $this->_longip <= $arr_beg[3] ) ) {
			...
			return $beg;
		} else if ( ( $this->_longip >= $arr_mid[2] ) && ( $this->_longip <= $arr_mid[3] ) ) {
			unset( $arr_beg );
			unset( $arr_mid );
			unset( $arr_end );

			return $mid;
		} else if ( ( $this->_longip >= $arr_end[2] ) && ( $this->_longip <= $arr_end[3] ) ) {
			unset( $arr_beg );
			unset( $arr_mid );
			unset( $arr_end );
		...
		} else if ( ( $this->_longip > $arr_beg[3] ) && ( $this->_longip < $arr_mid[2] ) ) {
			unset( $arr_beg );
			unset( $arr_mid );
			unset( $arr_end );
		...
		} else if ( ( $this->_longip > $arr_mid[3] ) && ( $this->_longip < $arr_end[2] ) ) {
			unset( $arr_beg );
			unset( $arr_mid );
			unset( $arr_end );
	...
	public function clear() {
		$this->_found        = false;
		$this->_start_ip     = false;
		$this->_end_ip       = false;
		$this->_country_code = false;
		$this->_country_name = false;
	}
}
