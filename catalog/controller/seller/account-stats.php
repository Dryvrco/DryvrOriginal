...
class ControllerSellerAccountStats extends ControllerSellerAccount {
	public function index() {
		...
		$seller_id = $this->session->data['seller_id'];
	...
	public function getByProductData() {
		...
		foreach($products as $p){
			$product_html = "<span class='name'><a href='" . $this->url->link('product/product', 'product_id=' . $p['product_id'], 'SSL') . "'>{$this->model_catalog_product->getMakeModelCategoriesYear($p['product_id'])}</a></span>";
			$columns[] = array_merge(
				$p,
				array(
