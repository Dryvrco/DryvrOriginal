      ...
      <div id="content" class="ms-product <?php echo $class; ?> ms-account-profile"><?php echo $content_top; ?>
         ...
         <form id="ms-sellerinfo" class="ms-form form-horizontal">
            ...
            <div class="form-group required">
               ...
               <div class="col-sm-10"> <b><?php echo $seller['ms.nickname']; ?></b> </div>
               <?php } else { ?>
               <label class="col-sm-2 control-label"><?php echo $ms_account_sellerinfo_nickname; ?></label>
               <div class="col-sm-10">
            ...
            <div class="form-group">
               ...
               <div class="col-sm-10">
                  <select name="seller[zone]" class="form-control" id="zone_id">
                  </select>
            ...
            <div class="form-group">
               ...
               <div class="col-sm-10" id="cities">
                  <select name="seller[city]" id="city_id" class="form-control">
                     <option>-- Select City --</option>
                     <?php mkdd($cities, $seller['ms.city_id']); ?>
                  </select>
               </div>
            </div>
            <div class="form-group">
               <label class="col-sm-2 control-label" for="input-country">Areas</label>
               <div class="col-sm-10" id="areas">
                     <?php foreach ($areas as $area){ ?>
                     <div class="col-sm-3">
                     <input type="checkbox" name="seller[area][]" value="<?php echo $area['area_id']; ?>" <?php if (in_array($area['area_id'], $area_array)) { echo "checked"; } ?> /> <?php echo $area['name']; ?>
                     </div>
                     <?php } ?>
               </div>
            </div>
               ...
               <div class="col-sm-10"> 
                  ...
                  <div id="sellerinfo_avatar_files">
                     ...
                     <div class="ms-image">
                        ...
                        <img src="<?php echo $seller['avatar']['thumb']; ?>" /> <span class="ms-remove"></span> </div>
                     <?php } ?>
                  </div>
               </div>
            </div>
            ...
            <div class="form-group">
               ...
               <div class="col-sm-10">
                  <div class="buttons"> <a name="ms-file-sellerbanner" id="ms-file-sellerbanner" class="btn btn-primary"><span><?php echo $ms_button_select_image; ?></span></a> </div>
                  ...
                  <div id="sellerinfo_banner_files">
                     ...
                     <div class="ms-image">
                        ...
                        <img src="<?php echo $seller['banner']['thumb']; ?>" /> <span class="ms-remove"></span> </div>
                     <?php } ?>
                  </div>
               </div>
            </div>
         ...
         <p class="alert alert-warning ms-commission"> <?php echo sprintf($this->language->get('ms_account_sellerinfo_fee_flat'),$this->currency->format($group_commissions[MsCommission::RATE_SIGNUP]['flat'], $this->config->get('config_currency')), $this->config->get('config_name')); ?> <?php echo $ms_commission_payment_type; ?> </p>
         <?php if(isset($payment_form)) { ?>
         <div class="ms-payment-form"><?php echo $payment_form; ?></div>
         <?php } ?>
         <?php } ?>
         <div class="buttons">
            <div class="pull-left"><a href="<?php echo $link_back; ?>" class="requestBtn"><span><?php echo $button_back; ?></span></a></div>
            <?php if ($seller['ms.seller_status'] != MsSeller::STATUS_DISABLED && $seller['ms.seller_status'] != MsSeller::STATUS_DELETED) { ?>
            <div class="pull-right">
               <button class="requestBtn" id="ms-submit-button"><?php echo $ms_button_save; ?></button>
            </div>
            <?php } ?>
         </div>
...
$(document).on("change", "#zone_id", function(){
	$.ajax({
      ...
      {
	  $('#cities').html('<select class="form-control" name="seller[city]" id="city_id"><option value="">-- Select City --</option>' + data + '</select>');
	  $('#areas').html('<select class="form-control" name="seller[area][]" id="area_id" multiple="multiple" size="7"><option value="" disabled="disabled">-- Select Area --</option></select><p class="ms-note">Select Multiple areas by holding down ctrl key.</p>');
	  } 
	});
});
...
$(document).on("change", "#city_id", function(){
	$.ajax({
		url: 'index.php?route=localisation/city/getCbAreas&token=<?php echo $token; ?>&city_id=' + this.value,
		type: 'POST',
        dataType: 'html',
        success : function(data)  
      {$('#areas').html(data);} 
