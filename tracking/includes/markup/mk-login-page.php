...
<head>
	...
	<title><?php echo isset($this->OPTIONS['brandname']) ? $this->OPTIONS['brandname'] : 'Heatmap Tracker'; ?></title>
	<link rel="shortcut icon" type="image/x-icon" href="<?php echo $this->getFavIcon(); ?>" />
	<link media="all" rel="stylesheet" type="text/css" href="<?php echo $this->PLUGIN_URL ?>css/bootstrap.css"/>
			...
			<div class="holder">
				...
				<form id="loginform" class="form-vertical no-padding no-margin" method="post" action="<?php echo admin_url() ?>">
					<div class="row">
						...
						<div class="text"><input required=""  id="input-username" type="text" placeholder="Email Address" name="username"/></div>
					</div>
					<div class="row">
						<label for="pass">Password:</label>

						<div class="text"><input required="" id="input-password" type="password" placeholder="Password" name="password"/></div>
					</div>
					<div class="row">
					...
					<div style="text-align: right; font-size: 10px; margin-top: 15px;"><?php echo ucwords($this->OPTIONS['heatmap_package']); ?> v<?php echo CURRENT_VERSION; ?></div>
