...
class ModelSaleCustomer extends Model {
    ...
    public function getTotalProducts($seller_id) {
        $sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "ms_product msp on (p.product_id = msp.product_id)";

        $sql .= " WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND msp.seller_id = '" . $seller_id . "'";
