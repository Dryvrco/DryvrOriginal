...
class ControllerSellerAccountProfile extends ControllerSellerAccount {
    ...
    public function jxSaveSellerInfo() {
        ...
        $seller = $this->MsLoader->MsSeller->getSeller($this->agency->getSellerId());
        $json = array();
        ...
        if (empty($json['errors'])) {
            ...
            if (empty($seller)) {
                    ...
                    case MsSeller::MS_SELLER_VALIDATION_APPROVAL:
                        ...
                        $mails[] = array(
                            ...
                            'data' => array(
                                ...
                                'customer_name' => $this->agency->getFirstname() . ' ' . $this->agency->getLastname(),
                                'customer_email' => $this->MsLoader->MsSeller->getSellerEmail($this->agency->getSellerId())
                            )
                        );
                ...
                $data['seller']['seller_id'] = $this->agency->getSellerId();
                ...
                if ($fee > 0) {
                    switch ($commissions[MsCommission::RATE_SIGNUP]['payment_method']) {
                        case MsPayment::METHOD_PAYPAL:
                            ...
                            $this->MsLoader->MsSeller->changeStatus($this->agency->getSellerId(), MsSeller::STATUS_UNPAID);
                        ...
                        default:
                            // deduct from balance
                            $this->MsLoader->MsBalance->addBalanceEntry($this->agency->getSellerId(), array(
    ...
    public function index() {
        ...
        $this->data['salt'] = $this->MsLoader->MsSeller->getSalt($this->agency->getSellerId());
        $this->data['statusclass'] = 'warning';

        $this->load->model('localisation/city');

        $this->data['cities'] = $this->model_localisation_city->getCitiesList($seller['ms.zone_id']);

        $this->data['areas'] = $this->model_localisation_city->getCbAreasList($seller['ms.city_id']);

        $this->data['area_array'] = explode(',', $seller['ms.area']);
