	...
	return {
				...
				var data = {
					...
					onLabelShow: function (event, label, code) {
						if (sample_data[code] != undefined)
							label.html(label.html() + ' Visits: ' + sample_data[code]);
					},
		...
		initCharts: function () {
			...
			function showTooltip(title, x, y, contents) {
				$('<div id="tooltip" class="chart-tooltip"><div class="date">' + dates[contents[1]] + '<\/div> ' + ((contents[2] == 0) ? (' <div class="label label-important">Sessions: ' + contents[0] + '<\/div>') : ('<div class="label label-info">Time: ' + time_format[contents[1]] + '<\/div>')) + '<\/div>').css({
					position: 'absolute',
					display: 'none',
					top: y - 100,
					width: 105,
			...
			$("#site_statistics").bind("plothover", function (event, pos, item) {
				...
				if (item) {
					if (previousPoint != item.dataIndex) {
						...
						showTooltip('24 Jan 2013', item.pageX, item.pageY, [item.datapoint[1], item.dataIndex, item.seriesIndex]);
					}
				} else {
					$("#tooltip").remove();
					previousPoint = null;
