...
final class MsSeller extends Model {
    ...
    public function __construct($registry) {
        ...
        if (isset($this->session->data['seller_id'])) {
            ...
            $seller_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ms_seller WHERE seller_id = '" . (int) $this->session->data['seller_id'] . "'");
        ...
        $sql = "INSERT INTO " . DB_PREFIX . "ms_seller
				SET seller_id = " . (int) $data['seller_id'] . ",
					seller_status = " . (isset($data['status']) ? (int) $data['status'] : self::STATUS_INACTIVE) . ",
                                        seller_featured = '" . (int) $data['featured'] . "',  
    ...
    public function editSeller($data) {
        ...
        if (isset($areas)) {
            $sql = "SELECT product_id FROM " . DB_PREFIX . "ms_product WHERE seller_id = '" . (int) $seller_id . "'";
            $res = $this->db->query($sql);
            foreach ($res->rows as $p_id) {
                $this->db->query("UPDATE " . DB_PREFIX . "product SET country_id = '" . (int) $data['country'] . "', zone_id = '" . (int) $data['zone'] . "', city_id = '" . (int) $data['city'] . "', area_id = '" . $areas . "' WHERE product_id = '" . $p_id['product_id'] . "'");
            }
        }
    ...
    public function getSeller($seller_id, $data = array()) {
        $sql = "SELECT	CONCAT(c.firstname, ' ', c.lastname) as name,
						...
						ms.seller_status as 'ms.seller_status',
                                                ms.seller_featured as 'ms.seller_featured', 
    ...
    public function getSellers($data = array(), $sort = array(), $cols = array()) {
        ...
        $sql = "SELECT
				...
				LEFT JOIN `" . DB_PREFIX . "ms_product` mp
					ON (c.customer_id = mp.seller_id)
				WHERE c.customer_id = c.agency_id "
    ...
    public function deleteSeller($seller_id) {
        ...
        $query = $this->db->query("SELECT customer_id FROM " . DB_PREFIX . "customer WHERE agency_id = '" . (int) $seller_id . "'");
        foreach ($query->rows as $id) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "ms_seller WHERE seller_id = '" . (int) $id['customer_id'] . "'");
        }
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE `query` = 'seller_id=" . (int) $seller_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int) $seller_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "customer WHERE agency_id = '" . (int) $seller_id . "'");
    }
