 ...
 */
	private function get_payment_address_view(){
		...
		$this->data['addresses'] = $this->model_module_slideshow->getAddresses($this->session->data['customer_id']);
		...
		if($this->cart->getProducts() || !empty($this->session->data['vouchers'])){
			...
			foreach ($products as $product) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$daily = $this->currency->format($this->tax->calculate($product['daily'], $product['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$daily = false;
				}
				...
				$this->data['products'][] = array(
					...
					'image'     => $image,
					'thumb'     => $image,
					'name'      => $this->model_catalog_product->getMakeModelYear($product['product_id']),
					'model'     => $product['model'],
					'option'    => $option_data,
					'recurring' => $recurring,
