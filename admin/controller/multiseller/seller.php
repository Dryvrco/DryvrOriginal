...
class ControllerMultisellerSeller extends ControllerMultisellerBase {
    ...
    public function getTableData() {
        ...
        foreach ($results as $result) {
            ...
            $actions .= "<a class='btn btn-primary' data-toggle='tooltip' href='" . $this->url->link('multiseller/seller/update', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'], 'SSL') . "' title='" . $this->language->get('button_edit') . "'><i class='fa fa-pencil'></i></a>";
            $actions .= "&nbsp;<a class='btn btn-danger' data-toggle='tooltip' href='" . $this->url->link('multiseller/seller/delete', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'], 'SSL') . "' title='" . $this->language->get('button_delete') . "' OnClick=\"return confirm('Are you sure?');\"><i class='fa fa-trash-o'></a>";
    ...
    public function jxSaveSellerInfo() {
        ...
        if (empty($json['errors'])) {
            ...
            if (empty($data['seller']['seller_id'])) {
                ...
                if (empty($data['customer']['customer_id'])) {
                    // creating new customer
                    $customer_id = $this->model_sale_customer->addCustomer(
                            array_merge(
                                    $data['customer'], array(
                        'telephone' => '',
                        'fax' => '',
                ...
                $seller_id = $this->MsLoader->MsSeller->createSeller(
                        array_merge(
                                $data['seller'], array(
                    'approved' => 1,
                                )
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "customer SET agency_id = '" . $seller_id . "' WHERE customer_id = '" . $customer_id . "'");
            } else {
                // edit seller
                $mails[] = array(
    ...
    public function update() {
        ...
        if (!empty($seller)) {
            ...
            $seller_info = $this->data['seller'];
            $this->data['seller_featured'] = $seller_info['ms.seller_featured'];
