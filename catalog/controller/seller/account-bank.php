...
class ControllerSellerAccountBank extends Controller {
    ...
    public function edit() {
        ...
        if ($this->model_account_bank->checkBank($this->request->get['bank_id'], $this->agency->getSellerId()) > 0) {
            $this->getForm();
        } else {
            $this->response->redirect($this->url->link('seller/account-bank'));
        }
    }
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('seller/account-bank')
        );
        ...
        $user_total = $this->model_account_bank->getTotalBanks();

        $results = $this->model_account_bank->getBanks($filter_data);
        ...
        foreach ($results as $result) {
            $data['banks'][] = array(
                ...
                'edit' => $this->url->link('seller/account-bank/edit&bank_id=' . $result['bank_id'] . $url),
                'delete' => $this->url->link('seller/account-bank/delete&bank_id=' . $result['bank_id'] . $url)
            );
        }
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/multiseller/account-bank-list.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/multiseller/account-bank-list.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/catalog/multiseller/account-bank-list.tpl', $data));
        }
    }
